...
// Function to delete search history from local storage
...
function pageInitialize() {
    ...
    $("#deleteHistoryButton").click(deleteSearchHistory);
    // Add click event listener to the search buttons
    $('.searchButton').on('click', function () {
        console.log("Clicked Search Button");
        var city = $("#searchBar").val();
        var state = $("#stateCode").val();
        console.log(city + ": from search click");
        console.log(state + ": from search click");
        if(city)
        {
    ...
    $("#searchBar").keydown(function (event) {
        if (event.keyCode === 13) {
            ...
            var city = $("#searchBar").val();
            var state = $("#stateCode").val();
            console.log(city + ": from enter");
            console.log(state + ": from enter");
            if(city)
            {
            getCurrentWeather(city, state);
            }
